...
def preporcess_chicago():
    df = pandas.read_csv(
        ...
        usecols=['Date', 'Description', 'Location Description', 'FBI Code', 'IUCR', 'Arrest'])   # + some local area
...
def preporcess_los_angeles():
    df = pandas.read_csv(
        ...
        usecols=['DATE OCC', 'AREA NAME', 'Crm Cd', 'Crm Cd Desc', 'Vict Age',
                 'Vict Sex', 'Vict Descent', 'Premis Desc', 'Status Desc'])

    df['Crime Code'] = df['Crm Cd'].apply(assign_nibris_code_la)
    df['Crime Category'] = df['Crime Code'].apply(translate_nibric_codes)

    df['Date'] = pandas.to_datetime(df['DATE OCC'], format='%m/%d/%Y %H:%M:%S %p')
...
def preprocess_new_york():
    df = pandas.read_csv(
        ...
        usecols=['CMPLNT_FR_DT', 'CMPLNT_FR_TM', 'KY_CD', 'OFNS_DESC', 'BORO_NM',
                 'PREM_TYP_DESC', 'SUSP_AGE_GROUP', 'SUSP_RACE',
    ...
    df = df[['Date', 'Crime Code', 'Crime Category', 'Local Crime Code',
             'Crime Description', 'Suspect Age Group', 'Suspect Sex',
